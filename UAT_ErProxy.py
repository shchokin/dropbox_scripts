...
for tag in tags:
    if tag.get('key') == 'ProxyURL':
        ...
        tag.set('value', f'http://erproxy.uat-1.kofile.systems/{tenant}/')
    ...
    if is_new_num_of_orders and is_new_num_of_oits:
        if er_type == "simple":
            content = f'''d=$(date "+%m_%d_%Y_%I_%M %p"); for i in {{1..{num_of_orders}}}; 
                          do cp ../../type_simple/{num_of_oits}oit.xml "ARTYOM-$d-$i.xml"; 
                          done;cd ../.. ; start erProxyTester.exe ; cd -'''
        else:
            content = f'''d=$(date "+%m_%d_%Y_%I_%M %p"); for i in {{1..{num_of_orders}}}; 
                          do cp ../../type_ds/{num_of_oits}oit.xml "ARTYOM-$d-$i.xml"; 
                          done;cd ../.. ; start erProxyTester.exe ; cd -'''
        print("\n'Generate Packages' file  is updated with new Number Of Orders and OITs")
    elif is_new_num_of_orders and not is_new_num_of_oits:
        if er_type == "simple":
            content = f'''d=$(date "+%m_%d_%Y_%I_%M %p"); for i in {{1..{num_of_orders}}}; 
                          do cp ../../type_simple/{config_num_of_oits[0]}oit.xml "ARTYOM-$d-$i.xml"; 
                          done;cd ../.. ; start erProxyTester.exe ; cd -'''
        else:
            content = f'''d=$(date "+%m_%d_%Y_%I_%M %p"); for i in {{1..{num_of_orders}}}; 
                          do cp ../../type_ds/{config_num_of_oits[0]}oit.xml "ARTYOM-$d-$i.xml"; 
                          done;cd ../.. ; start erProxyTester.exe ; cd -'''
...
'''
Acoound Code/Password check -> 1 func: check er_schema, pass, checkbox front office (is flag) 
Update Account Balance !!!
Specify Doc Type
Logs
Check status 4/8 - retrieving
Delete Retrived folder
'''
