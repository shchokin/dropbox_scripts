...
SELECT * from VANGUARD.TENANT where tenant_id = 26

select * from VANGUARD.dept where dept_id = 12

select * from [VANGUARD].[TENANT_DEPT] where tenant_id = 26 AND dept_id = 12
--update 'td_to_date' to 'CurrentDate' without time
--update [VANGUARD].[TENANT_DEPT] set td_to_date = CAST(GETDATE() AS Date) where dept_id = 12 and tenant_id = 26



select * from VANGUARD.SCHEDULE_JOB  where TENANT_ID=26 and  SCHEDULE_JOB_CODE like'%Nightly%'
--UPDATE VANGUARD.SCHEDULE_JOB SET IS_CANCELED=0 where TENANT_ID=26 and  SCHEDULE_JOB_CODE like'ExportNightly'
--update VANGUARD.SCHEDULE_JOB SET NEXT_EXECUTE_DATE= DATEADD(SECOND,2,GETDATE()) where TENANT_ID=26 and SCHEDULE_JOB_CODE like'ExportNightly'
...
UPDATE VANGUARD.SCHEDULE_JOB
   ...
   WHERE SCHEDULE_JOB_ID=28300


select * from VANGUARD.SCHEDULE_JOB_HISTORY
