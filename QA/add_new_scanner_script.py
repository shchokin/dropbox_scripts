...
conn = pymssql.connect(server=QA_DB.host, user=QA_DB.user,
                       password=QA_DB.password, database=QA_DB.database)
cursor = conn.cursor()
...
cursor.execute("select WORK_STATION_ID from VG" + str(tenant_code) +
               ".WORK_STATION where WORK_STATION_ADDRESS = '10.0.2.247' ")
result = cursor.fetchone()
