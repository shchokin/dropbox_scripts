...
vpn_connect("UAT")
# subprocess.call([r"D:\VOLO\Tools\UAT_VPN\Bat\vpn_uat_on.bat"])
...
while True:
    try:
        ...
        if not tenant:
            tenant = "48000"
            print("\nTenant " + colored("48000", "green") + " is selected by default")
            break
...
(
  select WORK_STATION_ID from VG{tenant}.WORK_STATION where WORK_STATION_ADDRESS = '{UAT_CRS.ip}' 
...
if not result:
    ...
    for child in second_root.iter("BaseAddress"):
        new_text = f"http://crs.uat-1.kofile.systems/{tenant}/capturemodule/api"
        # print(f"\nOld Url = {child.text}")
